...
app.get("/api/v2/sessions", async (c) => {
  try {
    ...
    if (data.errorCode !== 0) {
      return c.json(
        { error: "Error fetching sessions from Infinite Flight API" },
        500
      );
    }

    const expertServer = data.result.filter(
      (server) => server.name === "Expert"
    );
...
app.get("/api/v2/sessions/:session_id/flights", async (c) => {
  ...
  try {
    const response = await fetch(
      `https://api.infiniteflight.com/public/v2/sessions/${session_id}/flights`,
      {
        headers: {
          Authorization: `Bearer ${API_KEY}`,
        },
    ...
    if (data.errorCode !== 0) {
      return c.json(
        { error: "Error fetching flights from Infinite Flight API" },
        500
      );
    }
...
app.get("/api/v2/sessions/:sessionId/flights/:flightId/route", async (c) => {
  ...
  try {
    const response = await fetch(
      `https://api.infiniteflight.com/public/v2/sessions/${sessionId}/flights/${flightId}/route`,
      {
        headers: {
          Authorization: `Bearer ${API_KEY}`,
        },
    ...
    if (data.errorCode !== 0) {
      return c.json(
        { error: "Error fetching flight route from Infinite Flight API" },
        500
      );
    }
...
app.get(
  "/api/v2/sessions/:sessionId/flights/:flightId/flightplan",
  async (c) => {
    const sessionId = c.req.param("sessionId");
    const flightId = c.req.param("flightId");
    ...
    try {
      const response = await fetch(
        `https://api.infiniteflight.com/public/v2/sessions/${sessionId}/flights/${flightId}/flightplan`,
        {
          headers: {
            Authorization: `Bearer ${API_KEY}`,
          },
      ...
      if (data.errorCode !== 0) {
        return c.json(
          { error: "Error fetching flight plan from Infinite Flight API" },
          500
        );
      }
...
app.get("/api/simbrief", async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      return c.json(
        { error: "Failed to fetch data from SimBrief API" },
        response.status
      );
    }
    ...
    const runwayData = takeoffTlr["runway"].find(
      (runway) => runway["identifier"] === plannedRunway
    );
    const destinationRunwayData = landingTlr["runway"].find(
      (runway) => runway["identifier"] === destinationPlannedRunway
    );

    const routeMapUrl = images["directory"] + images["map"][0]["link"];
    const verticalProfileMap = images["map"].find(
      (image) => image["name"] === "Vertical profile"
    );
    const verticalProfileUrl = verticalProfileMap
      ? `https://www.simbrief.com/ofp/uads/${verticalProfileMap["link"]}`
      : null;
    ...
    return c.json(
      { error: `Error fetching flight plan: ${error.message}` },
      500
    );
  }
});
